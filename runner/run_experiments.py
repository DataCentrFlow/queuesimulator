...
template = './simulator 1 conf_{0}_split={1}_compres={2}_mar={3}.txt > result_{0}_split={1}_compres={2}_mar={3}.txt'
...
for mar in range(0, 4):
  for comr in range(1, 11):
    for split_mode in range(5, 6):
      for i in range(0, 2):
        rv = comr_scale * comr;
        conf_str = conf_str_template.format(queue_types[i], srpt_mode[i], split_mode, rv, pq_mode[i], mar)
        conf_file = "conf_{0}_split={1}_compres={2}_mar={3}.txt".format(names[i], split_mode, rv, mar)	
        with open(conf_file, 'w') as f:
          print conf_file
          f.write(conf_str)
          threads.append(threading.Thread(target=run_exp, args=((names[i], split_mode, rv, mar), semaphore)))

for mar in range(0, 4):
  for comr in range(2, 3):
    for split_mode in [1, 2, 3,4,6,7,8,9,10]:
      for i in range(0, 2):
        rv = comr_scale * comr;
        conf_str = conf_str_template.format(queue_types[i], srpt_mode[i], split_mode, rv, pq_mode[i], mar)
        conf_file = "conf_{0}_split={1}_compres={2}_mar={3}.txt".format(names[i], split_mode, rv, mar)	
