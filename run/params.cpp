...
void read_experiment_parameters(std::string conf_filename, uint32_t exp_type) {
    ...
    while (std::getline(input, line)) {
        ...
        else if (key == "srpt_mode")
        {
            lineStream >> params.srpt_mode;
        }
        else if (key == "congestion_compress")
        {
            lineStream >> params.congestion_compress;
        }
        else if (key == "flow_split_mode")
        {
            lineStream >> params.flow_split_mode;
        }
        else if (key == "pq_mode")
        {
            lineStream >> params.pq_mode;
        }
        else if (key == "srpt_with_fair")
        {
            lineStream >> params.srpt_wit_fair;
        }
        else if (key == "marriage_type")
        {
            lineStream >> params.marriage_type;
        }
