...
Queue* Factory::get_queue(
    ...
    switch(type) {
        ...
        case PQ_QUEUE:
            return new PQ_Queue(id, rate, queue_size, location);
        case FAIRNESS_DEQUE_QUEUE:
            return new FairnessDequeQueue(id, rate, queue_size, location);
        case RANDOM_QUEUE:
            return new Random_Queue(id, rate, queue_size, location);
